...
Lakehouse names are read from lakehouse_solution.yml (data-driven configuration).
...
This will:
1. Read lakehouse names from lakehouse_solution.yml
...
def get_workspace_id(environment: str) -> str:
    ...
    if environment not in lakehouse_config:
        # Show only valid environment keys (filter out non-dict values like 'lakehouses')
        valid_envs = [k for k, v in lakehouse_config.items() if isinstance(v, dict) and 'id' in v]
...
def setup_lakehouses(environment: str) -> None:
    ...
    print(f"\n{'='*70}")
    print(f"Summary: {created_count} lakehouse(s) created")
    print(f"{'='*70}")
